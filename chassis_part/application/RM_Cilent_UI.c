...
uint16_t Robot_cline_number;
/****************************************串口驱动映射************************************/
void UI_SendByte(unsigned char ch)
{
	usart6_tx_dma_enable(&ch,1);
...
void UI_Delete(u8 Del_Operate,u8 Del_Layer)
{
   ...
   datahead.Sender_ID=Robot_number;
   datahead.Receiver_ID=Robot_cline_number;                          //填充操作数据
...
void Float_Draw(String_Data *image,char imagename[3],u32 Graph_Operate,u32 Graph_Layer,u32 Graph_Color,u32 Graph_Size,u32 Graph_Digit,u32 Graph_Width,u32 Start_x,u32 Start_y,fp32 float_data)
{
   int i;
   for(i=0;i<3&&imagename[i]!='\0';i++)
