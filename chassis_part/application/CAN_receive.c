    ...
    switch (rx_header.StdId)
    {
        ...
        case CAN_TRIGGER_MOTOR_ID:
        {
            ...
            break;
				...
				//decoding process on Chassis Board
...
static CAN_TxHeaderTypeDef  gimbal_board_motor_feedback_message;
static uint8_t              gimbal_board_motor_rpm_feedback_data[8]={0};

void CAN_motor_feedback_send(int16_t chassis_motor1, int16_t chassis_motor2, int16_t chassis_motor3, int16_t chassis_motor4){
		uint32_t send_mail_box;
		
    gimbal_board_motor_feedback_message.StdId = chassis_motor_feedback_id;
    gimbal_board_motor_feedback_message.IDE = CAN_ID_STD;
    gimbal_board_motor_feedback_message.RTR = CAN_RTR_DATA;
    gimbal_board_motor_feedback_message.DLC = 0x08;
    gimbal_board_motor_rpm_feedback_data[0]=chassis_motor1>>8;
		gimbal_board_motor_rpm_feedback_data[1]=chassis_motor1;
		gimbal_board_motor_rpm_feedback_data[2]=chassis_motor2>>8;
		gimbal_board_motor_rpm_feedback_data[3]=chassis_motor2;
		gimbal_board_motor_rpm_feedback_data[4]=chassis_motor3>>8;
		gimbal_board_motor_rpm_feedback_data[5]=chassis_motor3;
		gimbal_board_motor_rpm_feedback_data[6]=chassis_motor4>>8;
		gimbal_board_motor_rpm_feedback_data[7]=chassis_motor4;
		
    HAL_CAN_AddTxMessage(&Communication_CAN, &gimbal_board_motor_feedback_message, gimbal_board_motor_rpm_feedback_data, &send_mail_box);
}
...
void CAN_heat_data_send(uint16_t shooter_heat0,uint16_t shooter_heat1){
    ...
    HAL_CAN_AddTxMessage(&Communication_CAN, &gimbal_board_heat_message, gimbal_board_heat_send_data, &send_mail_box);
}
...
void CAN_shoot_data_send(uint8_t	bullet_type, uint8_t	bullet_freq, float	bullet_speed){
    ...
    HAL_CAN_AddTxMessage(&Communication_CAN, &gimbal_board_heat_message, gimbal_board_shoot_data, &send_mail_box);
}
